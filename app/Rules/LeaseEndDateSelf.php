...
class LeaseEndDateSelf implements Rule
{
    protected $unit_id;
     ...
     * @return void
     */
    public function __construct($unit_id, $start_date, $lease_id)
    {
    ...
    public function passes($attribute, $value)
    {
        ...
        if(!empty($this->end_date) && ($this->start_date > $this->end_date)){
            $this->fullMessage = "End Date shouldn't be less than Start Date";
            $success = false;
            return $success;
        }

        //Check if we have payments or bills after the entered date
        ...
        $query1->where('payments.pay_date', '>=', $this->end_date);
        ...
        $query->where('bills.due_date', '>=', $this->end_date);
        $billsCount = $query->count();

        if(($leasePaymentsCount > 0) || ($billsCount > 0)){
            $this->fullMessage = "You can't change Lease End date because there was payment(s) made or there was bill(s) created during the specified date range.";
            $success = false;
            return $success;
        }
