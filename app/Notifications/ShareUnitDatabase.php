...
class ShareUnitDatabase extends Notification
{
    use Queueable;
    ...
    public function toArray($notifiable)
    {
        if (is_object($this->tenant)){
            return [
                'type' => 'UNIT_SHARED',
                'url' => $this->unit->public_link,
                'creator_user_id' => $this->landlord->id,
                'user_id' => $notifiable->id,
                'description' => ' about ' . $this->property->full_address . ", " . $this->unit->name,
                'title' => "Landlord would like to share information",
            ];
        } else {
            return [];
        }
