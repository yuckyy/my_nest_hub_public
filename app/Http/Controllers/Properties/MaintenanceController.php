...
{
    public function dashboard(Request $request, int $unit)
    {
        $u = Unit::find($unit);
        ...
        foreach ($maintenanceRequestsNew as $main) {
            ...
            $qq4 = DB::table('expense_types')->where('id', '=', $main->category)->orderBy('id', 'desc')->first();
            $main->property_address = $qq3->address;
            if ($qq->deleted_at) {
                $main->deleted_at = $qq->deleted_at;
            } else {
                $main->deleted_at = null;
            }
            if ($qq4 !== null) {
                $main->expense_type = $qq4->name;
            } else {
                $main->expense_type = null;
            }
        ...
        $queryEx = DB::table('expenses');
        $queryEx->where('expenses.unit_id', '=', $unit);
        $expensesCount = $queryEx->count();
        ...
        return view('properties.units.maintenance.dashboard', [
    ...
    public function listView(Request $request, int $unit)
    {
        $query = MaintenanceRequest::query();
        ...
        $servicePross = ServicePro::all(['id', 'user_id', 'first_name', 'last_name', 'middle_name', 'company_name', 'company_website', 'email', 'phone', 'fax', 'tax_identity_type', 'tax_payer_id', 'display_as_company', 'category', 'street_address', 'city', 'state_region', 'zip', 'country']);
        $servicePros = $servicePross->where('user_id', '=', Auth::user()->id);
        return view('properties.units.maintenance.list-view', [
            'servicePros' => $servicePros,
            'allcategory' => $allExCategory,
            'allsubcategory' => $allExSubCategory,
    ...
    public function viewDetails(Request $request, int $unit)
    {
    ...
    public function add(Request $request)
    {
        ...
        if ($request->get('expense_type') == '_new') {
            ...
            if ($expenseType) {
                $maintenanceRequest->category = $expenseType->id;
            } else {
                $expenseType = new ExpenseType();
                $expenseType->name = $request->get('expense_name');
                $expenseType->pid = $request->get('pid');
                $expenseType->user_id = Auth::user()->id;
                $expenseType->save();
                $qq = DB::table('expense_types')->where('user_id', '=', Auth::user()->id)->orderBy('id', 'desc')->first();
                $maintenanceRequest->category = $qq->id;
            }

        } else {
            $maintenanceRequest->category = intval($request->get('expense_type'));
        }
        ...
        $result = view('properties.units.maintenance.maintenance_request', [
