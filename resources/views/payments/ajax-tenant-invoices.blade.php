...
<div class="card dashboardCard mb-4">
    ...
    <form action="{{ route('pay-invoice') }}" method="get">
                        ...
                        <tr>
                            <th>
                                @if ($lease->landlordLinkedFinance())
                                    <div class="form-check form-check-inline">
                                        <input name="invoice-all" type="checkbox" value="" class="form-check-input" id="invoice-all" title="Select All" {{ ($lease->trashed() || session('adminLoginAsUser')) ? 'disabled' : '' }}>
                                    </div>
                                @endif
                            </th>
                    ...
                    @foreach($invoices as $invoice)
                        <tr>
                            <td class="actionIconsBlock oneIcon">
                                @if($invoice->balance < 0 && ($lease->landlordLinkedFinance() || !empty($lease->landlordPayPalFinancial())))
                                    <div class="form-check form-check-inline">
                                        <input name="invoice[]" type="checkbox" value="{{ $invoice->id }}" class="form-check-input invoice-single" id="invoice-{{ $invoice->id }}" {{ ($lease->trashed() || session('adminLoginAsUser')) ? 'disabled' : '' }}>
                                    </div>
                                @endif
                            </td>
                            <td>
                            ...
                            <td class="actionIconsBlock wide200">
                                <span class="showViewModal" data-toggle="modal" data-target="#detailsModal" data-id="{{ $invoice->id }}">
        ...
        <div class="card-footer pb-0 pt-3 ">
            <div class="float-left mb-3 withTooltip bg-white p-1" data-toggle="tooltip" data-placement="top" @if (!$lease->landlordLinkedFinance() && empty($lease->landlordPayPalFinancial())) title="Please contact your landlord" @else title="To make a payment check at least one checkbox">
                <button type="submit" class="btn btn-primary btn-sm btn-pay-invoices" disabled @endif ><i class="fal fa-check mr-2"></i> Pay Selected Invoices</button>
            </div>
