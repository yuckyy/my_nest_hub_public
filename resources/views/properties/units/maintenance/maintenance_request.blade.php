...
{{--                <span data-toggle="modal" data-target="#addExpenseModal" data-record-id="{{ $maintenanceRequest->id }}" data-record-title="{{ '#' . $maintenanceRequest->id . " - " . $maintenanceRequest->name }}" >--}}
{{--                    <button title="Archive Ticket" class="btn btn-sm btn-light mr-1 text-black"><i class="fal fa-dollar-sign"></i> Add Expense</button>--}}
{{--                </span>--}}
            @endif
            <span class="vacant-check-click"data-toggle="modal" data-target="#detailsModal"  data-property-address="{{ $maintenanceRequest->property_address }}" data-vacant="{{ $maintenanceRequest->deleted_at }}"  data-record-id="{{ $maintenanceRequest->id }}" data-color="{{ $maintenanceRequest->color() }}" data-record-title="{{ '#' . $maintenanceRequest->id . ", " . date('m/d/Y', strtotime($maintenanceRequest->created_at)) }}" >
